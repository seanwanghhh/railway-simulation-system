...
final class PlatformUtils {
    ...
    public static final boolean openBrowserAtPath(String path) {
        ...
        try {
            ...
            runtime.exec(new String[]{command, path});
            return true;
        } catch (IOException e) {
            System.err.println("Failed to open browser (" + command + " " + path + ")");
            e.printStackTrace(System.err);
            String wslMessage = String.format("If you are using WSL, you will need to navigate to %s manually", path);
            System.out.println(RED_BOLD + wslMessage + ANSI_RESET);
            return false;
        }
    }
